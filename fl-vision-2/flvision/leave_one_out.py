...
from flvision.contribution_utils import ContributionTracker, weighted_average
from flvision.model_poison import generate_contribution_attack
from flvision.contribution_strategy import LeaveOneOutStrategy, TorchGTGShapleyStrategy

PROJECT_NAME = "FL-Contribution-Analysis-Fashion"
...
class ContributionFedAvg(FedAvg):
    ...
    def __init__(self, holdout_test_loader, use_wandb=False, run_config=None, 
        ...
        if self.contrib_method == "shapley" or self.contrib_method == "gtg-shapley":
            ...
            self.contrib_strategy = LeaveOneOutStrategy(Net, test)
    ...
    def _apply_attack_if_needed(self, rnd, results):
        ...
        if attacker_idx is not None and rnd in self.attack_rounds and hasattr(self, "current_global_params"):
            ...
            # Generate malicious parameters using the external function
            # Capture the returned hyperparameters, including Shapley params used in attack
    ...
    def aggregate_fit(self, rnd, results, failures):
        ...
        global_loss, global_acc = test(global_model, self.holdout_test_loader, self.device)
