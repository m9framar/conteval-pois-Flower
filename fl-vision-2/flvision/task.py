...
def load_data(partition_id: int, num_partitions: int, partition_type="iid", dirichlet_alpha=1.0):
    """Load partition FashionMNIST data."""
    # Only initialize `FederatedDataset` once
    global fds
    if fds is None:
        # Choose partitioner based on configuration
        if partition_type == "dirichlet":
            partitioner = DirichletPartitioner(
                num_partitions=num_partitions,
                partition_by="label",
                alpha=dirichlet_alpha,  # Use the provided alpha
                seed=42,
