...
class LeaveOneOutStrategy(ContributionEvaluationStrategy):
    ...
    def evaluate_contribution(self, 
        ...
        for i in range(num_clients):
            ...
            from flvision.task import set_weights  # Import here to avoid circular imports
            set_weights(model, loo_params)
            loo_loss, loo_acc = self.evaluate_fn(model, test_loader,self.device)
...
class TorchGTGShapleyStrategy(ContributionEvaluationStrategy):
    ...
    def evaluate_contribution(self, 
        ...
        def utility_fn(subset):
            ...
            # Evaluate model
            model = self.model_cls()
            from flvision.task import set_weights  # Corrected import
            set_weights(model, numpy_params)
            loss, _ = self.evaluate_fn(model, test_loader, self.device)
            return loss
